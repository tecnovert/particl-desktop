...
def open_wallet(wallet_name, mnemonic):
    wallets = cli1('listwallets')
    print('wallets', wallets)
    if wallet_name not in wallets:
        try:
            cli1(f'loadwallet {wallet_name}')
        except Exception:
            cli1(f'createwallet {wallet_name}')
            cli1('walletsettings stakingoptions "{\\"stakecombinethreshold\\":100,\\"stakesplitthreshold\\":200}"', wallet_name)
            cli1('walletsettings stakelimit "{}"'.format(dumpje({'height': 1})), wallet_name)
            cli1('reservebalance true 10000000', wallet_name)
            cli1(f'extkeyimportmaster "{mnemonic}"', wallet_name)
...
def main():
    ...
    print('\nPreparing wallets')
    open_wallet('main', 'abandon baby cabbage dad eager fabric gadget habit ice kangaroo lab absorb')
    open_wallet('stakingwallet', 'assume bubble junior pulp bridge casino try merge sure appear seed robust')
    open_wallet('spendingwallet', 'injury junk seed chat maple tower illegal drive history rug umbrella rival')

    staking_account = cli1('extkey account', 'stakingwallet')
    print('[rm] staking_account', staking_account)
    for c in staking_account['chains']:
        if c['function'] == 'active_external':
            external_chain = c['chain']
            break

    changeaddress = {'coldstakingaddress': external_chain}
    cli1('walletsettings changeaddress "{}"'.format(dumpje(changeaddress)), 'spendingwallet')
